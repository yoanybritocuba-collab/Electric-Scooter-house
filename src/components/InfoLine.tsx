...
const InfoLine = () => {
  ...
  const [cyclePhase, setCyclePhase] = useState<'animando' | 'pausa'>('animando');
  ...
  // CICLO DE ANIMACIÓN: 5 pasadas + 6 segundos de pausa
  useEffect(() => {
    if (!info?.activo || !visible) return;

    const startCycle = () => {
      setCyclePhase('animando');
      
      // Calcular duración de 5 pasadas completas
      const fivePassesDuration = (info.velocidad || 60) * 5 * 1000; // en milisegundos
    ...
    return cleanup;
  }, [info?.activo, visible, info?.velocidad]);
    ...
    <AnimatePresence>
      ...
      >
        <div className="relative w-full h-full flex items-center">
          {/* Contenido animado - solo visible en fase 'animando' */}
          {cyclePhase === 'animando' && (
            <div
              ref={marqueeRef}
          ...
          {/* Durante la pausa - línea vacía o con efecto sutil */}
          {cyclePhase === 'pausa' && (
            <div className="absolute inset-0 flex items-center justify-center opacity-30">
              <span className="text-xs tracking-widest" style={{ color: info.colorTexto }}>
                • • •
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="absolute right-4 z-10 text-white/80 hover:text-white transition-colors bg-black/20 hover:bg-black/30 rounded-full p-1"
          ...
          {/* Enlace si existe */}
          {info.link && cyclePhase === 'animando' && (
            <a
              href={info.link}
              target="_blank"
              rel="noopener noreferrer"
              className="absolute left-4 z-10 bg-white/20 hover:bg-white/30 px-3 py-1 rounded-full text-xs transition-colors backdrop-blur-sm"
              style={{ color: info.colorTexto }}
            >
