...
const Index = () => {
  ...
  const Section = ({ title, icon: Icon, items, emptyMessage }: { 
    title: string; 
    icon?: any; 
    items: Product[];
    emptyMessage?: string;
      ...
      return (
        <section className="py-16">
          <div className="flex items-center gap-3 mb-8">
            {Icon && <Icon size={24} className="text-primary opacity-50" />}
            <h2 className="font-display font-bold text-2xl md:text-3xl tracking-tight text-foreground opacity-50">
              {title}
            </h2>
          </div>
    ...
    <div className="min-h-screen">
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          ...
          {/* Sección de Más Vendidos - AHORA CON MENSAJE SI ESTÁ VACÍA */}
          <Section 
            title={t("home.best_sellers")} 
            icon={Star} 
            items={masVendidosList} 
            emptyMessage="Próximamente productos destacados"
          />

          {/* Sección de Ofertas - AHORA CON MENSAJE SI ESTÁ VACÍA */}
          <Section 
            title={t("home.on_sale")} 
            icon={Percent} 
            items={ofertasList} 
            emptyMessage="No hay ofertas disponibles por el momento"
          />
        </div>
      </div>
    </div>
